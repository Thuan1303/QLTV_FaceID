...
# url = "http://10.190.194.183:8080/shot.jpg"
...
def xuat_cam(name):
    #Doc file vua duoc tranning 
    recognizer.read(f"./data/classifiers/{name}_classifier.xml") 
    # Do chinh xac 1 la thanh cong ,-1 là thất bại
    pred = 0
    # img_resp = requests.get(url)
    # img = np.array(bytearray(img_resp.content),dtype= np.uint8)
    # img = cv2.imdecode(img,-1)
    
    ret,img = cap.read()#mo cam mera de dectec
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)#dua anh vua doc duoc sang mau sac bth
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)#dua anh vua doc duoc sang mau sac hợp lý để face
    faces = face_cascade.detectMultiScale(gray,1.3,5)#tìm kuon mat thong qua file haarcascade_frontalface_default.xml
    for (x,y,w,h) in faces:
        roi_gray = gray[y:y+h,x:x+w]
        id,confidence = recognizer.predict(roi_gray) #du_doan khuon mat thong qua file dc tranning
        confidence = 100 - int(confidence)#do tin cay
        if confidence > 50:
            pred = 1
            text = name
